...
export const bindAccountForm = (supabase, sessionState, showToast) => {
  ...
  const reloadAccountPage = (success, showCardForm = false) => {
    const params = new URLSearchParams();
    if (success) params.set("success", success);
    if (showCardForm) params.set("addCard", "1");
    window.location.href = buildPageUrl("account.html", params);
  };
  ...
  if (new URLSearchParams(window.location.search).get("addCard") === "1") {
    openCardForm();
  }
  ...
  accountForm.addEventListener("submit", async (event) => {
    ...
    const textUpdatesInput = document.getElementById("textUpdates");
    const emailUpdatesInput = document.getElementById("emailUpdates");
    const allowSavedCardInput = document.getElementById("allowSavedCard");
    const textUpdates =
      textUpdatesInput instanceof HTMLInputElement ? textUpdatesInput.checked : sessionState.profile.textUpdates;
    const emailUpdates =
      emailUpdatesInput instanceof HTMLInputElement ? emailUpdatesInput.checked : sessionState.profile.emailUpdates;
    const allowSavedCard =
      allowSavedCardInput instanceof HTMLInputElement ? allowSavedCardInput.checked : sessionState.profile.allowSavedCard;
    ...
    try {
      ...
      if (cardAction === "remove-card") {
        ...
        reloadAccountPage("card-removed", true);
        return;
      }
