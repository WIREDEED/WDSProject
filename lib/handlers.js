...
export const bindAccountForm = (supabase, sessionState, showToast) => {
  ...
  accountForm.addEventListener("submit", async (event) => {
    ...
    if (password && password.length < 6) {
      showToast("error", "Your new password must be at least 6 characters.");
      return;
    }
    ...
    try {
      ...
      showToast(
        "success",
        password ? "Your account changes were saved. Your password was updated." : "Your account changes were saved."
      );
    } catch (error) {
