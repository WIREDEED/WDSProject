...
export const bindReviewForm = (supabase, sessionState, showToast) => {
  ...
  reviewForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      // Silent Success Logic for double-submits
      if (orderError) {
        if (orderError.message.includes("unique_order_prevention") || orderError.code === '23505') {
      ...
      // Foreign Key safety: Stop if order creation failed entirely
      ...
      // 4. Handle Loyalty Transactions (The Ledger)
      if (customerType === "registered" && sessionState.profile) {
        const loyaltyEntries = [];

        // Redeming Points (Spend)
        if (discountTier) {
          loyaltyEntries.push({
            user_id: userId,
            order_id: orderId,
            points_change: -discountTier.points,
            description: `${discountTier.points} points redeemed for discount on order #${orderId}`
          });
        }

        // Earning Points (Calculated 10%)
        loyaltyEntries.push({
          user_id: userId,
          order_id: orderId,
        ...
        // Batch Insert with duplicate protection check
        const { error: loyaltyTxError } = await supabase
            .from("loyalty_transactions")
            .insert(loyaltyEntries);

        if (loyaltyTxError && !loyaltyTxError.message.includes("duplicate key")) throw loyaltyTxError;
