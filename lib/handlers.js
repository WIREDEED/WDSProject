...
export const bindWalletForm = (supabase, sessionState, showToast) => {
  ...
  walletForm.addEventListener("submit", async (event) => {
    ...
    const amount = money(formData.get("addAmount"));
    const fundingSource = String(formData.get("fundingSource") || "");

    if (!amount || amount <= 0) {
      showToast("error", "Please choose an amount to add.");
      return;
    }

    if (!fundingSource) {
      showToast("error", "Please choose how you want to fund the wallet.");
      return;
    }
    ...
    try {
      const { error: updateError } = await supabase
        .from("users")
        .update({ wallet_balance: money(sessionState.profile.walletBalance + amount) })
        .eq("user_id", sessionState.profile.userId);

      if (updateError) throw updateError;

      const { error: txError } = await supabase.from("wallet_transactions").insert({
        user_id: sessionState.profile.userId,
        order_id: null,
        transaction_type: "deposit",
        amount,
        description: `Wallet deposit from ${fundingSource.replace("-", " ")}`
      });

      if (txError) throw txError;

      showToast("success", "Your wallet deposit was saved.");
      window.location.reload();
    } catch (_error) {
      showToast("error", "Could not add funds right now.");
    }
  });
};
  ...
  reviewForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (customerType === "registered") {
        ...
        if (paymentMethod === "wallet" && sessionState.profile.walletBalance < total) {
          throw new Error("There is not enough money in your wallet for this order yet.");
        }
      ...
      if (customerType === "registered" && sessionState.profile) {
        ...
        if (paymentMethod === "wallet") {
          updates.wallet_balance = money(sessionState.profile.walletBalance - total);
