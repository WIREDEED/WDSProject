...
export const isProtectedPage = () =>
  ["dashboard.html", "wallet.html", "account.html"].includes(getCurrentFileName());
...
export const populateSessionFields = (sessionState) => {
  ...
  const fields = {
    ...
    dashboardWalletBalance: `$${profile.walletBalance.toFixed(2)}`,
    dashboardLoyaltyPoints: `${profile.loyaltyPoints} Loyalty Points`,
    walletPageBalance: `$${profile.walletBalance.toFixed(2)}`,
    accountName: profile.fullName || "",
    accountPhone: profile.phone || "",
    accountEmail: profile.email || "",
    accountWalletBalance: `$${profile.walletBalance.toFixed(2)}`,
